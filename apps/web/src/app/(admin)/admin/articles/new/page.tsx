...
export default function NewArticlePage() {
  ...
  const coverState = useDocumentUpload({ category: 'article-covers' })
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await createArticle({
        ...
        cover_url:    coverState.url || undefined,
  ...
  return (
    <div className="p-4 lg:p-8 pt-6 max-w-2xl">
      ...
      <h1 className="text-xl font-bold text-gray-900 mb-6">สร้างบทความใหม่</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          <div>
            ...
            <input value={form.title} onChange={e => set('title', e.target.value)} placeholder="ชื่อบทความ"
          ...
          <div>
            ...
            <div className="flex items-center gap-0 border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500">
              ...
              <input value={form.slug} onChange={e => handleSlugChange(e.target.value)} placeholder="article-slug"
          ...
          <div>
            ...
            <textarea value={form.excerpt} onChange={e => set('excerpt', e.target.value)}
              placeholder="สรุปย่อ (แสดงในหน้ารายการบทความ)" rows={2}
          ...
          <div>
            ...
            <textarea value={form.content} onChange={e => set('content', e.target.value)}
              placeholder="เนื้อหาบทความทั้งหมด" rows={10}
              className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
          </div>
          ...
          <div className="flex items-center gap-3 py-2 border-t border-gray-100">
            ...
            <div>
              <p className="text-sm font-medium text-gray-700">{form.is_published ? 'เผยแพร่ทันที' : 'บันทึกเป็นฉบับร่าง'}</p>
              <p className="text-xs text-gray-400">{form.is_published ? 'บทความจะแสดงในหน้าสาธารณะ' : 'ยังไม่แสดงต่อสาธารณะ'}</p>
            </div>
          </div>
        </div>
