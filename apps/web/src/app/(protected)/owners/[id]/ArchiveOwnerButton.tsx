...
import { archiveOwner, restoreOwner } from '../actions'
...
export default function ArchiveOwnerButton({ ownerId, isArchived }: Props) {
  ...
  function handleArchive() {
    ...
    startTransition(async () => {
      const res = await archiveOwner(ownerId)
  ...
  function handleRestore() {
    setError(null)
    startTransition(async () => {
      const res = await restoreOwner(ownerId)
      if (res.error) setError(res.error)
    })
  }
  ...
  if (confirm) {
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 space-y-2">
        <p className="text-xs text-amber-800 font-medium">เก็บถาวรเจ้าของ?</p>
        <p className="text-xs text-amber-700">เจ้าของจะไม่ปรากฏในรายการปกติ แต่ยังสามารถกู้คืนได้ภายหลัง</p>
        {error && <p className="text-xs text-red-600">{error}</p>}
